...
bool is_not_prime[100];
vector<long long> primes(100);
...
long long countWays(long long n){
    long long primeIdx = 0;
    while(n > 1 && primes[primeIdx]<=n){
        if(n%primes[primeIdx] == 0){
            n /= primes[primeIdx];
            primeSet.insert(primes[primeIdx]);
        }
        else{
            primeIdx++;
        }
...
void solve(){
    ...
    SCLLD(A); SCLLD(B);
    primeSet.clear();
    long long count1 = countWays(A*B);
    long long count2 = primeFactors(A*B);
    cout << count2 << endl;
