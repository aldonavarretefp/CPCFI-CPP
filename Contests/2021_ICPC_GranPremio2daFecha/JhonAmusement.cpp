...
void solve(){
    int64 n, amusmentDuration; 
    cin>> n >> amusmentDuration;
    vector<project> p;
    // debug(p.size());
    FO(i,n){
        int64 start, startings, reward, duration;
        cin>> reward >> duration >> startings;
        // debug(reward,duration,startings);
        for(int64 j = 0; j < startings; j++){
            project newProject;
            cin >> start;
            newProject.start = start;
            newProject.end = start + duration;
            newProject.reward = reward;
            // debug(newProject.start,newProject.end,newProject.reward);
            p.push_back(newProject);
        }
    }
