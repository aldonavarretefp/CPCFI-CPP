...
#define isOn(S, j) (S & (1 << j)) //AND Verifica si está prendido el bit
#define setBit(S, j) (S |= (1 << j)) // OR 
#define clearBit(S, j) (S &= ~(1 << j)) // AND NEGADA 
#define toggleBit(S, j) (S ^= (1 << j)) //XOR 
#define lowBit(S) (S & (-S)) //
#define setAll(S, n) (S = (1 << n) - 1) // Recorre n y le resta 1,
