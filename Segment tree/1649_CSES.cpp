...
long long T(int node, int i, int j, int L, int R) {
    if (L > j || R < i) return INF; //RMaxQ
    if (L >= i && R <= j) return ST[node];
    int M = (L+R)/2;
    return min(T(node*2, i, j, L, M), T(node*2+1, i, j, M+1, R));
}
void update(int64 pos, int newVal, int n){
    A[pos] = newVal;
    //En el segment tree estaría en la posicion n+pos
    ST[n+pos] = newVal;
    //Vamos al padre y lo actualizamos hasta que de 0, que es antes de la raíz
    int64 i = (n+pos)/2;

    while(i>0){
        //Actualiza a sus padres
