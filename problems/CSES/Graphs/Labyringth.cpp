...
void solve(){
    ...
    while (!q.empty()){
        ...
        q.pop();
        FO(i, 4){
            int x = ccoord.first + dx[i];
            int y = ccoord.second + dy[i];
            if(!outOfBounds(x, y) && mat[x][y] != '#' && !vis[x][y]){
                vis[x][y] = true;
                previ[x][y] = i;
                q.push(MP(x, y));
            }
    ...
    if (vis[end.first][end.second]) {
        ...
        vector<int> steps;
		while (end != start) {
			int p = previ[end.first][end.second];
			steps.push_back(p);
