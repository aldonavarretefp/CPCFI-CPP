...
private:
  int V;          // No. vertices
  vector<edge> EL; //EdgeLess
  vector<vi> AL; //Adjacency List
  vi d, last; //Dinic's
  vector<ii> p; //helper to backtrack augmenting path
  ...
  ll send_one_flow(int s, int t, ll f = INF) {   // send one flow from s->t
    ...
    ll pushed = send_one_flow(s, u, min(f, cap-flow));    // returns the bottleneck capacity
    flow += pushed;
...
public:
  ...
  void add_edge(int u, int v, ll w, bool directed = true) {
    ...
    AL[u].push_back(EL.size()-1);                // remember this index
     ...
     // back edge
    EL.emplace_back(u, directed ? 0 : w, 0);     // cap 0 directed, cap w undirected
...
int main() {
  ...
  freopen("1695_CSES_Police_Chase.in", "r", stdin);
  freopen("1695_CSES_Police_Chase.out", "w", stdout);
